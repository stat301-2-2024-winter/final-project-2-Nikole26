...
The motivation behind this project stems from my deep curiosity in understanding the dynamics between financial institutions and emerging enterprises, particularly the challenges start-ups face in securing loans. As an economics undergraduate interested in applying data science skills and knowledge to real-world business scenarios, this aspect of investigating the relationship between banks and start ups caught my attention. By delving into these interactions, this project provides a meaningful way for me to apply my data science expertise in economics.
...
The best performance of each model between the two recipes will be selected, and from them the best parameters will be determined for each and analysed.

#### 1. Elastic Net

The best penalty parameter is 0.0000000001, indicating that the model heavily penalizes the coefficients of the predictors. Thus, considering that the penalty is already at a very small value, further exploration might not be necessary. Besides, by lowering the penalty more to see if it improves model performance could  sacrifice too much interpretability.

#### 2. K-Nearest Neighbors

The best number of neighbors for the KNN model is 5, compared when it is set to 10. This indicates that a lower value for neighbors which led to more localized decision boundaries and a better performance.

#### 3. Boosted Trees

The number of variables randomly sampled at each split (`mtry`) that performed best when building the trees is 11 compared when it is set to 10. This means that higher `mtry` value typically leads to more diverse trees, potentially capturing a wider range of features and increasing the model's ability to generalize.

The best minimum amount of observations (`min_n`) required in a terminal node of the tree is 16 compared to when it is set to 600. This indicates a lower `min_n` allows for more splits, potentially capturing finer details and nuances in the data. 

Finally, both configurations have a best learning rate of 1.00, which means that the contribution of each tree in the boosting ensemble is not scaled. It would be recommend to explore higher learning rates can lead to faster convergence, while keeping in mind that it could also increase the risk of overfitting.

#### 4. Random Forest

The best number variables sampled at each split (`mtry`) when growing trees in a Random Forest, is 7 compared when it is set to 15. This means that a lower `mtry` value can lead to more diverse trees, potentially capturing a broader range of features that would improve the model performance.

The minimum number of observations (`min_n`) required in a terminal node of the tree that performed best when is 5 compared when it is set to 27. This suggests that a lower `min_n` value allows for more splits, which serves to the advantage to the random forest model's performance. However, setting it too low might lead to overfitting.
...
The model correctly predicted 3,742 instances as Charged Off (CHGOFF). These are the instances where the model predicted that the loans would be charged off, and they were indeed charged off.

* False Negatives (FN): 

The model incorrectly predicted 30 instances as Paid In Full (PIF) when they were actually Charged Off (CHGOFF). These are the instances where the model failed to predict that the loans would be charged off when they actually were.

* False Positives (FP): 

The model incorrectly predicted 8 instances as Charged Off (CHGOFF) when they were actually Paid In Full (PIF). These are the instances where the model predicted that the loans would be charged off, but they were actually paid in full.

* True Negatives (TN): 

The model correctly predicted 3,720 instances as "PIF" Paid In Full (PIF). These are the instances where the model predicted that the loans would be paid in full, and they were indeed paid in full.
...
@fig-best-model-accuracy shows the accuracy of the final model,the Random Forest model, is 0.9949333. This indicates that the model performed exceptionally well on the test dataset, meaning that it correctly predicted the outcome for approximately 99.49% of the instances in the test dataset.
...
As evidenced in @fig-roc, the area under the ROC curve (AUC) is close to 1. This indicates that the model has excellent discriminatory power, meaning it can effectively distinguish between the two classes, in this case, Charged Off and Paid in Full.
...
Through this comprehensive data exploration, methodical model building, and rigorous evaluation, the following conclusions have been drawn:

* Even though Recipe 2 included additional preprocessing steps compared to Recipe 1, such as handling correlated predictors and normalizing numeric predictors, the improvement in model performance was marginal. This suggests that the added complexity may not always translate into significantly better predictive performance.

* While Recipe 2 included additional preprocessing steps, it did not lead to a substantial improvement in model performance compared to Recipe 1. Thus, the choice of model type appears to have a larger impact on performance, with ensemble-based models generally outperforming simpler models.

*  Random Forest consistently emerged as the top-performing model, outperforming other models in terms of accuracy. Its ability to capture complex relationships in the data contributed to its superior predictive performance.

* The Random Forest model's high accuracy and excellent discriminatory power would significant practical implications for loan approval decisions. Its ability to accurately classify loan applications as "Charged Off" or "Paid in Full" would minimize the risk of approving risky loans, while also ensuring that viable applicants are not unfairly rejected.

Future work and next steps may involve exploring additional features or alternative modeling techniques to further enhance predictive performance. I would also suggest investigating the generalization of the best model, the random forest, across different geographical regions or loan types, since it could broaden its applicability and relevance in diverse contexts. Overall, this project lays a solid foundation for continued research and innovation in the area of loan approval prediction modeling.


## References

Toktogaraev, M. (2022). "Should This Loan be Approved or Denied? [Data set]. Kaggle. [https://www.kaggle.com/datasets/mirbektoktogaraev/should-this-loan-be-approved-or-denied?select=Should+This+Loan+be+Approved+or+Denied+A+Large+Dataset+with+Class+Assignment+Guidelines.pdf](https://www.kaggle.com/datasets/mirbektoktogaraev/should-this-loan-be-approved-or-denied?select=Should+This+Loan+be+Approved+or+Denied+A+Large+Dataset+with+Class+Assignment+Guidelines.pdf)
