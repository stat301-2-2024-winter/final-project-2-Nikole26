...
####  Elastic Net
The best penalty parameter was set to a very small value of 0.0000000001, indicating that the model heavily penalizes the coefficients of the predictors. While the mixture parameter is set to 1, indicating that the model is a pure Elastic Net, with equal mixing between L1 and L2 penalties.
...
####  K-Nearest Neighbors
The best number of neighbors chosen for the KNN model was 5, which means that the model predicts the outcome based on the 5 nearest neighbors in the feature space. This number of neighbors used in recipe 2 perform better than the 10 neighbors used in recipe 1. This is because .....


####  Boosted Trees
The number of variables randomly sampled at each split when building the trees was set to 11 for the model using recipe 1, and it performed better than when this parameter was set to 10 for the model using recipe 2. Hence, it could be inferred that for this dataset used in this project selecting a higher amount of features  leads to better model performance by focusing on the most important predictors.

The minimum number of observations required in a terminal node of the tree. It's set to 16.
Learn_rate: The learning rate controls the contribution of each tree to the final prediction. A value of 1.00 indicates full learning from each tree.
