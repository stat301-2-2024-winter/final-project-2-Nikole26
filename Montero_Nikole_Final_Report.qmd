...
To deal with the missing values, I decided to downsample my data by focusing on the target variable, the loan status, which has two categories: "Charged Off" (CHGOFF) or "Paid in Full" (PIF). Thus, I consider 15,000 observations for each category, achieving the downsample of my dataset to 30,000 observations. This introduced an artificial balance of the binary target variable, as seen in FIGURE COMPLETE, which will address potential imbalances in the original data distribution.   
